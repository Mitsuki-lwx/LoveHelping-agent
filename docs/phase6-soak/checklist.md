...
- [x] J13 ~~**S10a 成立**（用既有产物即可证）：稳态段闸门在途峰值 S9a **8** / S9b **21**，
      ...
      ⚠️ **后续更正（同日）**：这条"成立"**不成立** —— 该判据是**恒真命题**
      （`inFlight()` 被 clamp 到 `ceiling ≤ max-inflight`），**永远不可能失败**。
      详见 K 段与 spec §5.4.2
...
- [x] J16 **本轮不实现 S10a–S10e**：属独立工作流，且 Mitsuki 已明确不追厂商配额
      ...
      ⚠️ **后续更正（同日）**：其中 **S10a 零新增输入即可实现，已改为本轮实现**，见 K 段；
      J16 仅对 S10b–S10e 成立

## K. S10a 判据核查与撤回（2026-09-20）—— 一次自我推翻

> J16 曾把 S10a 划为"不实现"。复核后我判断"零新增输入即可实现"，于是动手实现。
> **实现后实测判失败，追下去发现原判据本身是恒真命题** —— 判据无效，实现已撤回。
> 本段**保留全过程（含我的错误结论）**，因为"判据是否有效"这件事本身需要证据。

### K.A 我的错误结论（保留在案）

- [x] K1 定位参照值来源缺口：`OnlineLoadTracker` 只暴露 `online.inflight.current` /
      `online.queue.depth` / `online.inflight.wait_ms`，**没有任何表示上限的指标**
- [x] K2 决策：不在脚本里复制配置常量（会漂移）→ 在应用侧补**只读** gauge
- [x] K3 ~~新增 `online.inflight.max`~~ → **命名错误，已改名**（见 K6）
- [x] K4 按原判据实现判据项 `s10a_gate_alignment` = `{limit, steady_inflight_peak, …}`

### K.B 实测暴露的三个问题

- [x] K5 5 分钟短压（32 worker）实测：`s10a_gate_alignment` **判失败**
      （`limit: 17.0`、`steady_inflight_peak: 21.0`）
- [x] K6 **根因一：gauge 命名错误**。`maxInFlight()` 返回的是**动态 `ceiling`**
      （源码注释原文即"准入天花板（动态）"），**不是**配置上限。
      把它叫 `.max` 会误导读指标的人 → **已改名 `online.inflight.ceiling`**
- [x] K7 **根因二：断言写法错误**。拿**末值** `limit` 去比**整段峰值** ——
      `ceiling` 是时变量，不能用单点值当常量参照，否则是苹果比橘子
- [x] K8 **根因三（最关键）：原判据是恒真命题**。`inFlight()` 返回
      `Math.max(0, Math.min(ceiling, used))` —— **被 clamp 到 `ceiling`**，
      而 `ceiling ≤ maxInFlight` 结构性成立 → **在途永远不可能超过配置上限**。
      实测 74 个稳态点**违例 0 个** —— 注意这不是"对齐良好"，而是**恒为 0**
- [x] K9 后果：该判据抓不到它本该抓的"对齐被破坏"。极端情形 —— 若 `CapacityLimitChanged`
      接线断掉，准入天花板停在 24 而网关已降到 15，多放的请求在网关层被拒，
      **而在途仍 ≤ 24 → S10a 照样判过**
- [x] K10 **撤回**：`soak.py` 的 `s10a_gate_alignment` 判据项与 `inflight_max` 采样键**已删除**

### K.C 修订方向（**未实现**）

- [x] K11 正确判据 = **两层闸门对齐**：稳态段 `online.inflight.ceiling` 不得高于网关上限
      `llm.permits.limit`（允许事件传播的短暂滞后）。**这条可以失败，因此有信息量**
- [x] K12 修订口径实测（用同一轮数据即可判）：稳态 74 点中 `ceiling > llm_limit`
      **0 个（0.0%）**，两者取值集合**完全相同**（6…21），末值均为 17 → **对齐成立**
- [ ] K13 **未实现**修订判据 —— 待定：**滞后容差取多少**（事件传播期间 ceiling 短暂高于
      llm_limit 属正常；本轮 5s 采样未见滞后，**需在 30 分钟轮确认**）

### K.D 保留的产出

- [x] K14 保留可观测性：`OnlineLoadTracker` 新增只读 gauge **`online.inflight.ceiling`**
      （一行 `Gauge.builder`，**不改任何准入行为、不动任何参数**）——
      它补的是"ceiling 此前完全不可观测"这个缺口，也是 K11 修订判据的前置条件
- [x] K15 DoD：编译 + 单测 **220/220，BUILD SUCCESS**；E2E 冒烟 `e2e_live.py` **22/22 PASS**；
      改名前后各确认一次指标名（改名前 `online_inflight_max`、改名后 `online_inflight_ceiling`）
- [x] K16 文档同步：`docs/08` §2.2 指标登记表补新 gauge（**用正确名字**），
      并标注"闸门族指标未统一登记"这一已知缺口（**只标缺口，不静默替别人补齐**）
