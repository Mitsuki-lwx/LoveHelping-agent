...
> 原表述"若 429 > 0，说明闸门对齐被破坏"——把厂商限流**只当成并发维度**。

**实测事实**：厂商限流有**两个独立维度**——
1. **并发**上限 ≈ 24（§8.8 直连实测，瞬时口径，超额立即 429）；
2. **速率**配额（bigmodel `code 1302`「您的账户已达到速率限制」/ dashscope `Throttling.RateQuota`），
   本轮日志中 429 出现 **616** 次、`code 1302` **198** 次。

闸门取 24 = 并发上限**本身** → **零余量**；再叠加重试（`llm_retry_total{scheduled}` 549）
与记忆抽取等辅助调用**共享同一网关额度**，持续负载下瞬时需求会周期性顶破**速率**配额。
...
| S10b | 厂商 429 | **闸门对齐 且 降级链可用**时 = 0；否则按上游归因单独报告，**不判应用失败**，但必须记录 |
| S10c | 降级链健康 | 主/备供应商至少一条可用；两条同时不可用 ⇒ 判为**环境阻塞**，整轮结论标注"未验收" |

**本轮结论**：S10a 通过；S10b 不通过（429 来自速率配额）；S10c 不通过（dashscope 网络不可达）。
⇒ 本轮**不判"全部通过"**，S10 待环境恢复后重跑关闭。原 S10 结果**保留在案**，不覆盖。
