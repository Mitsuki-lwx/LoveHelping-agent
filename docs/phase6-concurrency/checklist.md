...
- [x] E6 **30 分钟混合负载稳定性（2026-09-16 执行，`docs/phase6-soak/` + `docs/09` §8.9）**：
      应用侧 **12/13 通过** —— 在途贴住闸门 24 未越界、队列恒为 8 无漂移、Hikari pending/timeout 全 0、
      冷却后归零（permit 无泄漏）、堆无单调增长、稳态 5xx = 0、闸门硬拒率 3.07%。
      **S10"厂商 429 = 0"不通过**（上游降级 86.66%），归因厂商**速率**配额（`code 1302`）
      + 降级供应商 dashscope 不可达，**非应用缺陷**（零 LLM 护栏场景 1478/1478 全通为反证）。
      口径已修订：闸门取 24 = 厂商**并发**上限本身 ⇒ 零余量，S10 需"降级链可用"才成立
      （`docs/phase6-soak/spec.md` §5.1）。**待环境恢复后重跑关闭 S10。**
