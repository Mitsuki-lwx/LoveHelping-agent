...
| 131 篇精选情感内容构建知识库（含 60 篇中文语境方法论文：恋爱脑机制 / 吵架急救 / 表白攻略 / 谈钱对齐 / 风险识别 / 生育决策 / 心理危机应对等），**混合检索**（jieba 分词 RRF + pgvector 语义召回），回答附来源引用与打分 | 一张图自动分发：闲聊 / 知识问答 / 工具任务全自动（`OrchestrationGraph`），**用户永远只需要"写一封信"** | 预置 8 种人设 + 自定义性格/关系阶段创建沙盘会话，模拟对话用于沟通排练；每个 TA 有独立记忆上下文 |
...
> ✅ 首次启动自动完成：MySQL Flyway 迁移（V1-V19）、PostgreSQL 建表 + **131 篇文档自动向量化**（增量按 doc_hash，重启只同步变更）。
...
flowchart LR
  ...
  N --> V[(PG 16 + pgvector<br/>131 篇情感知识库)]
  A --> R[(Redis)]
...
- 🎯 **检索评测**：45 例 ground-truth，生产基线 **Recall 0.98 / MRR 0.82**（2026-09-11 库扩充至 131 篇后复测；召回未退化，MRR 因语义近邻增多、排序被稀释——重排优化见 `docs/phase5-kb-expansion/checklist.md` 待办）
