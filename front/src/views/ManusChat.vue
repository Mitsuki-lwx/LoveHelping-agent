  ...
  <div class="chat-page">
    ...
    <div class="chat-messages" ref="messagesRef">
      <div v-if="messages.length === 0" class="empty-state">
        ...
        <p>你好！我是恋爱帮帮帮，<br>知行合一，帮你解决恋爱中的任何问题！</p>
      </div>
      <div
      ...
      >
        ...
        <div v-if="msg.role === 'ai' && !loading" class="vote-row">
          <button :class="['vote-btn', voteStates[idx] === 'LIKE' ? 'vote-active-like' : '']" @click="vote(idx, 'LIKE')" title="有用">👍</button>
          <button :class="['vote-btn', voteStates[idx] === 'DISLIKE' ? 'vote-active-dislike' : '']" @click="vote(idx, 'DISLIKE')" title="没用">👎</button>
          <input v-if="showFeedback === idx" v-model="feedbackText" class="feedback-input" placeholder="为什么没用？（可选）" @keydown.enter="submitFeedback(idx)" @blur="submitFeedback(idx)" />
        </div>
      </div>
...
function sendMessage() {
  ...
  inputText.value = ''
  loading.value = true
  saveLocalConversation(currentSessionId.value, text)
  registerConversation(currentSessionId.value, text.substring(0, 50), 'manus').catch(() => {})
  forceScrollToBottom()

  messages.value.push({ role: 'ai', content: '' })
  const aiMsgIdx = messages.value.length - 1

  cancelSSE = createManusChatSSE(text, currentSessionId.value, {
    onMessage(data) {
      const cleaned = data.replace(/^Step \d+ result:\s*/, '')
      if (cleaned) {
        messages.value[aiMsgIdx].content += cleaned
      }
      scrollToBottom()
    },
    onError() {
...
function stopMessage() {
  if (currentSessionId.value) {
    stopManusChat(currentSessionId.value)
  }
  cancelSSE?.()
  loading.value = false
}
...
function vote(msgIdx, type) {
  ...
  if (voteStates.value[msgIdx] === type) {
    ...
    voteMessage(currentSessionId.value, msgIdx, 'NEUTRAL', '')
    return
  }
  if (type === 'LIKE') {
    voteStates.value[msgIdx] = 'LIKE'
    showFeedback.value = null
    voteMessage(currentSessionId.value, msgIdx, 'LIKE', '')
    return
  }
...
function submitFeedback(msgIdx) {
  ...
  voteMessage(currentSessionId.value, msgIdx, 'DISLIKE', feedbackText.value)
  showFeedback.value = null
  feedbackText.value = ''
}
</script>
